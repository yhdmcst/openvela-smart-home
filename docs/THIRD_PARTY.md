...
| 依赖名称 | 版本 | 许可证 | 用途 | 上游仓库 |
|----------|------|--------|------|----------|
| openvela (NuttX-based) | - | Apache-2.0 | 嵌入式 RTOS 与 AIoT 运行环境 | https://github.com/open-vela |
| Vela AI Agent / MimiClaw derived code | repository snapshot | Apache-2.0 / MIT | Agent 核心、通道与工具框架 | 具体归属见仓库 `NOTICE` 与源码文件头 |
...
| 依赖名称 | 版本 | 许可证 | 用途 |
|----------|------|--------|------|
| GCC (aarch64-linux-gnu) | ≥ 10 | GPLv3+ (with Runtime Library Exception) | ARM64 交叉编译 |
...
1. 本项目不分发完整 openvela 工作区或 QEMU 二进制；`auto_build_vela.sh` 会在用户环境中准备相关依赖和源码。
